...
from fetch import fetch_threads, add_thread_to_db, update_thread_topic
...
def add_thread(thread_id, topic_name="New Chat"):
    ...
    if thread_id not in st.session_state["thread_topics"]:
        st.session_state["thread_topics"][thread_id] = topic_name
        add_thread_to_db(thread_id, topic_name)
...
if "chat_threads" not in st.session_state:
    ...
    st.session_state["chat_threads"] = [t["id"] for t in threads]
    st.session_state["thread_topics"] = {t["id"]: t["topic"] for t in threads}

if "thread_topics" not in st.session_state:
    st.session_state["thread_topics"] = {}

# Ensure current working thread exists
...
if user_input:
    ...
    if st.session_state["thread_topics"][st.session_state["thread_id"]] == "New Chat":
        rename_prompt = (
            "Summarize this user request into a short, clear conversation title "
            "(max 5 words, no quotes): " + user_input
        )
        ...
        update_thread_topic(st.session_state["thread_id"], title)
